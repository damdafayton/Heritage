...
export async function deriveKey(password: string, salt = new Uint8Array([1])) {
...
export async function encryptText(key: CryptoKey, text: string, test = false) {
  const crypto = getCrypto();
...
export async function decryptText(key: CryptoKey, ciphertext: string) {
  const crypto = getCrypto();
